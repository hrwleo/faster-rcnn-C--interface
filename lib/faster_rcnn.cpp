 ...
 * =====================================================================================
 */
void Detector::vis_detections(cv::Mat image, int* keep, int num_out, float* sorted_pred_cls, float CONF_THRESH, int cls_idx)
{
	int i=0;
 	string str;
	if(cls_idx == 1){str = "no";}
	if(cls_idx == 2){str = "yes";}
	if(cls_idx == 3){str = "face";}
	if(cls_idx == 4){str = "phonecall";}
	if(cls_idx == 5){str = "notcall";}
	while(sorted_pred_cls[keep[i]*5+4]>CONF_THRESH && i < num_out)
	{
		if(i>=num_out)
			return;
		cv::putText(image, str, cv::Point(sorted_pred_cls[keep[i]*5+0], sorted_pred_cls[keep[i]*5+1]), CV_FONT_HERSHEY_COMPLEX, 1, cv::Scalar(255, 0, 0));
